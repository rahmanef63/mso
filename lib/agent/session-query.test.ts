...
describe("full-store agent session resolver", () => {
  ...
  it("resolves the human agent-context label without requiring an internal id", () => {
    const rows = [row(1, "Workflow Session Graph", "2026-09-01T12:00:00Z")];
    expect(chooseAgentSessionRecord(rows, "test-1-workflow-session-graph").title).toBe("Workflow Session Graph");
  });
  ...
  it("supports unique fuzzy titles but refuses ambiguous matches using human labels", () => {
    ...
    expect(() => chooseAgentSessionRecord(rows, "Example Project")).toThrow(/test-1-deploy-example-project.*test-2-audit-example-project|test-2-audit-example-project.*test-1-deploy-example-project/);
  });
});
