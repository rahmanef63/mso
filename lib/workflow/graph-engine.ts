...
import type { WorkflowGraph, WorkflowGraphRun, WorkflowGraphRunEdgeState } from "@/lib/contracts/workflow-graph";
...
const state = globalThis as typeof globalThis & { msoGraphInstance?: string; msoGraphPending?: Map<string, Promise<void>>; msoGraphCancelled?: Set<string> };
const INSTANCE = state.msoGraphInstance ??= randomUUID();
const pending = state.msoGraphPending ??= new Map<string, Promise<void>>();
const cancelled = state.msoGraphCancelled ??= new Set<string>();
...
async function execute(run: WorkflowGraphRun, graph: WorkflowGraph, input: Record<string, unknown>, context: CapabilityRunContext, resolve: Resolver): Promise<void> {
  const rows = new Map(run.nodes.map((row) => [row.id, row])), outputs: Record<string, unknown> = {}, secrets = await workflowSecretValues(context.principal ?? context.actor ?? "");
  const activeEdges = graph.edges.filter((edge) => !edge.disabled);
  const edgeState = new Map(graph.edges.map((edge) => [edge.id, edge.disabled ? "disabled" as WorkflowGraphRunEdgeState : run.edges?.find((row) => row.id === edge.id)?.state ?? "pending" as WorkflowGraphRunEdgeState]));
  // Keep disabled incoming edges as disabled dependencies so a node whose only route is inactive is skipped, not promoted to a new root. Outgoing resolution touches active edges only.
  const incoming = new Map(graph.nodes.map((node)=>[node.id,graph.edges.filter((edge)=>edge.target===node.id)])), outgoing = new Map(graph.nodes.map((node)=>[node.id,activeEdges.filter((edge)=>edge.source===node.id)]));
  const setEdge=(id:string,state:WorkflowGraphRunEdgeState)=>{edgeState.set(id,state);const row=run.edges?.find((item)=>item.id===id);if(row)row.state=state;};
  const disableOutgoing=(id:string)=>{for(const edge of outgoing.get(id)??[])if(edgeState.get(edge.id)==="pending")setEdge(edge.id,"disabled");};
  const resolveOutgoing=(id:string,handles?:string[])=>{for(const edge of outgoing.get(id)??[]){const enabled=handles?Boolean(edge.sourceHandle&&handles.includes(edge.sourceHandle)):!edge.sourceHandle;setEdge(edge.id,enabled?"enabled":"disabled");}};
  const sessionEvent=(state:string,detail:string)=>appendAgentSessionEvent(context.principal ?? context.actor ?? "",run.sessionId,{kind:"workflow",tool:"workflow_graph",state,workflowId:graph.id,detail}).catch(()=>undefined);
  let handledErrors=false;
  try {
    const remaining=new Set(graph.nodes.map((node)=>node.id));
    while(remaining.size){
      if(cancelled.has(run.id)){run.state="interrupted";run.error="Stopped by user.";for(const pendingId of remaining){const queued=rows.get(pendingId);if(queued?.state==="queued"){queued.state="skipped";queued.logs.push("Skipped because the execution was stopped.");}}throw new Error(run.error);}
      ...
      for(const node of graph.nodes){
        ...
        row.durationMs=Date.now()-started;row.finishedAt=new Date().toISOString();run.updatedAt=row.finishedAt;await writeWorkflowGraphRun(run);await sessionEvent(row.state,node.name);remaining.delete(node.id);progressed=true;
      }
  ...
  } catch(error){if(run.state!=="failed"&&run.state!=="interrupted"){run.state="failed";run.error=secretText(redactText(error instanceof Error?error.message:"workflow graph failed",800),secrets);}}
  run.finishedAt=run.updatedAt=new Date().toISOString();await writeWorkflowGraphRun(run);await sessionEvent(run.state,graph.name);
}
...
export async function startWorkflowGraph(graph: WorkflowGraph, rawInput: unknown, idempotencyKey: string, context: CapabilityRunContext, resolve: Resolver, storagePrincipal?: string, trigger?: WorkflowRunTrigger, ancestry: string[] = []) {
  ...
  return lockWorkflowGraphRun(owner,id,async()=>{const existing=await readWorkflowGraphRun(owner,id);if(existing){if(existing.fingerprint!==fingerprint)throw new Error("idempotency_key already identifies a different graph/input/revision");return publicWorkflowGraphRun(existing);}await pruneWorkflowGraphRuns(owner);const run:WorkflowGraphRun={version:1,id,owner,sessionId:context.sessionId!,graphId:graph.id,graphRevision:graph.revision,graphName:graph.name,idempotencyKey,fingerprint,state:"running",startedAt:new Date().toISOString(),updatedAt:new Date().toISOString(),pid:process.pid,instance:INSTANCE,nodes:graph.nodes.map((node)=>({id:node.id,name:node.name,type:node.type,state:"queued",logs:[]})),edges:graph.edges.map((edge)=>({id:edge.id,source:edge.source,target:edge.target,...(edge.sourceHandle?{sourceHandle:edge.sourceHandle}:{}),state:edge.disabled?"disabled":"pending"})),trigger:trigger??{type:"manual",nodeId:graph.nodes.find((node)=>node.type==="manual")?.id??graph.nodes.find((node)=>["schedule","webhook","channel_trigger"].includes(node.type))?.id,receivedAt:new Date().toISOString()},ancestry:[...ancestry,graph.id],runtimeInput:structuredClone(input)};await writeWorkflowGraphRun(run);await appendAgentSessionEvent(principal,run.sessionId,{kind:"workflow",tool:"workflow_graph",state:"started",workflowId:graph.id,detail:graph.name}).catch(()=>undefined);const work=execute(run,graph,input,context,resolve).then(()=>maybeRunErrorWorkflow(run,graph,context,resolve,principal)).catch(()=>undefined).finally(()=>{pending.delete(id);cancelled.delete(id);});pending.set(id,work);return publicWorkflowGraphRun(run);});
}
...
export async function requestWorkflowGraphStop(principal:string,id:string){const owner=workflowGraphOwner(principal),run=await readWorkflowGraphRun(owner,id);if(!run)throw new Error("workflow graph run not found for this principal");if(run.state!=="running")return{...publicWorkflowGraphRun(run),stopRequested:false};if(run.instance!==INSTANCE||!pending.has(id))throw new Error("workflow execution is owned by another runtime; refresh its status before retrying stop");cancelled.add(id);return{...publicWorkflowGraphRun(run),stopRequested:true};}
