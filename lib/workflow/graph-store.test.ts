...
process.env.OS_AGENT_MEMORY_DIR = path.join(dir, "memory");
const store = await import("./graph-store");
const memory = await import("@/lib/agent/memory-store");
...
beforeAll(() => undefined);
afterAll(async () => { delete process.env.OS_AGENT_MEMORY_DIR; await rm(dir, { recursive: true, force: true }); });
...
describe("private workflow graph store", () => {
  it("isolates principals and enforces revisions", async () => {
    ...
    const updated = await store.updateWorkflowGraph("principal-a", created.id, created.revision, { ...graph, id: created.id, name: "Updated" }, "update", { remember: true });
    expect(updated.name).toBe("Updated");
    const remembered = await memory.queryAgentMemory("principal-a", { query: "workflow save updated", limit: 10 });
    expect(remembered.records).toHaveLength(1);
    expect(remembered.records[0].record.value).toContain("Event: workflow_save");
    expect(remembered.records[0].record.value).toContain("Workflow: Updated");
    expect(remembered.records[0].record.value).toContain("Nodes: 2");
  });
