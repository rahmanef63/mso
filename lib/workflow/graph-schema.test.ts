...
describe("workflow graph schema", () => {
  ...
  it("persists edge style/active state and ignores disabled edges for topology", () => {
    const parsed = parseWorkflowGraphDefinition({
      ...base(),
      edges: [
        { id: "e1", source: "start", target: "done", style: "dashed", disabled: true },
        { id: "e2", source: "done", target: "start", style: "solid", disabled: true },
      ],
    });
    ...
    expect(() => parseWorkflowGraphDefinition({ ...base(), edges: [{ id: "bad", source: "start", target: "done", style: "dots" }] })).toThrow("edge style");
  });
