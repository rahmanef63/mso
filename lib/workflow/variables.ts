...
async function read(owner: string): Promise<Store> { try { const target=file(owner), stat=await fs.lstat(target); if(!stat.isFile()||stat.isSymbolicLink()||stat.size>256*1024||(stat.mode&0o077)) throw new Error("unsafe workflow variable store"); const row=JSON.parse(await fs.readFile(target,"utf8")) as Store; if(row.version!==1||row.owner!==owner||!row.values||typeof row.values!=="object") throw new Error("invalid workflow variable store"); return row; } catch(error){ if((error as NodeJS.ErrnoException).code==="ENOENT") return {version:1,owner,values:{}}; throw error; } }
async function write(store: Store){ const target=file(store.owner),dir=path.dirname(target); await fs.mkdir(dir,{recursive:true,mode:0o700}); const body=JSON.stringify(store),tmp=`${target}.${randomUUID()}.tmp`; if(Buffer.byteLength(body)>256*1024) throw new Error("workflow variable store too large"); try{await fs.writeFile(tmp,body,{flag:"wx",mode:0o600});await fs.rename(tmp,target);}finally{await fs.unlink(tmp).catch(()=>undefined);} }
