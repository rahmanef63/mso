  ...
  for (const owner of selected) {
    try {
      ...
      graphs.push(...store.graphs.map(g => ({ id: g.id, name: g.name, status: g.status, project: g.metadata.project,
        updatedAt: g.updatedAt, originPrincipal: store.principal, owner, readOnly: store.principal !== principal, nodeCount: g.nodes.length })));
...
export async function cloneOwnerGraph(role: string, principal: string, owner: string, id: string) {
  requireDiscoveryOwner(role);
  const store = await readWorkflowGraphStore(owner);
  ...
  return createWorkflowGraph(principal, { ...graph, id: undefined, name: `${graph.name} Copy`.slice(0, 160), status: "draft",
    metadata: { ...graph.metadata, provenance: "clone", fingerprint: undefined } });
}
