...
describe("Jev Integration Variable resolution",()=>{
  it("uses JEV by default and preserves an explicit connection override",async()=>{
    ...
    await expect(resolveJevIntegrationConfig()).resolves.toEqual({user:"alice",connection:"jev"});
    await expect(resolveJevIntegrationConfig({variable:"JEV",model:"decision-model"})).resolves.toEqual({user:"alice",connection:"jev",model:"decision-model"});
    await expect(resolveJevIntegrationConfig({user:"alice",connection:"alternate"})).resolves.toEqual({user:"alice",connection:"alternate"});
    await expect(resolveJevIntegrationConfig({user:"alice"})).rejects.toThrow("both integration user and connection");
  });
  it("rejects a variable that points to a non-MCP provider",async()=>{
    const {integrationManage}=await import("@/lib/infra/connection-manage");
    await integrationManage({action:"user.create",confirm:true,user:"alice"});
    ...
    const {resolveJevIntegrationConfig}=await import("./jev-integration");
    await expect(resolveJevIntegrationConfig()).rejects.toThrow("must reference provider mcp");
  });
});
