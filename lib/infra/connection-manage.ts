...
export async function integrationManage(input:Record<string,unknown>){
  ...
  return mutateIntegrationState(state=>{
    ...
    if(action==="user.rename"||action==="user.duplicate"){
      ...
      for(const rows of Object.values(copy.connections))for(const c of Object.values(rows)){clearGoogleRuntime(c);c.uid=randomUUID();c.revision++;c.updatedAt=Date.now();delete c.lease;if(action==="user.duplicate"&&input.copyCredentials!==true){c.values={};delete c.verifiedAt;}if(action==="user.duplicate"&&c.source!=="direct"){c.external=undefined;delete c.verifiedAt;}}
    ...
    if(action==="connection.duplicate"){
      if(c.sharedFrom)throw new IntegrationError("shared_alias_copy_from_owner",409);
      const target=identity(input.target,"target"),targetUser=state.users[target];if(!targetUser)throw new IntegrationError("user_not_found",404);
      const targetConnection=identity(input.targetConnection??connection,"target_connection"),rows=targetUser.connections[provider]??{};if(Object.hasOwn(rows,targetConnection))throw new IntegrationError("connection_exists",409);
      const clone=structuredClone(c);clone.id=targetConnection;clone.uid=randomUUID();clone.label=connectionLabel(input.label??c.label);clone.values=c.source==="direct"&&input.copyCredentials===true?{...c.values}:{};delete clone.sharedFrom;delete clone.external;delete clone.verifiedAt;delete clone.lastCheck;delete clone.lease;clearGoogleRuntime(clone);clone.revision=1;clone.createdAt=clone.updatedAt=Date.now();
      targetUser.connections[provider]={...rows,[targetConnection]:clone};if(input.makeDefault===true||!targetUser.defaults[provider])targetUser.defaults[provider]=targetConnection;return{ok:true,action,connection:summaryIn(state,target,clone)};
    }
    if(action==="connection.share"){
      if(isGoogleProvider(provider)||provider===GOOGLE_APP_PROVIDER)throw new IntegrationError("google_shared_authorization_not_supported",409);
    ...
    if(action==="connection.delete"){
      if(provider===GOOGLE_APP_PROVIDER&&Object.values(profile.connections).some(rows=>Object.values(rows).some(x=>isGoogleProvider(x.provider)&&x.values.appConnection===connection)))throw new IntegrationError("google_app_has_connections",409);
    ...
    if(action==="credential.delete"){
      ...
      if(input.key!==undefined){const key=identity(input.key,"key");if(!connectionMethod(provider,c.source,c.authMethod).fields.some(f=>f.key===key))throw new IntegrationError("unknown_credential_field");delete c.values[key];}else c.values={};clearGoogleRuntime(c);c.revision++;delete c.verifiedAt;delete c.lastCheck;c.updatedAt=Date.now();
    }
