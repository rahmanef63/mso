...
.integration{max-width:var(--settings-content-width);margin:auto;font:var(--type-small)/var(--line-body) var(--font-body);color:var(--foreground)}
.integration h2{font:var(--weight-bold) var(--type-title)/var(--line-title) var(--font-display);margin:0 0 var(--space-2)}.integration p{color:var(--muted-foreground);margin:var(--space-2) 0 var(--space-3)}
.integration .setup-grid{display:grid;grid-template-columns:minmax(0,1fr) minmax(0,1fr);gap:var(--space-5)}.integration label{display:flex;align-items:baseline;gap:var(--space-2);flex-wrap:wrap;font-weight:var(--weight-strong);margin-bottom:var(--space-1)}.integration label .field-source{font-size:var(--type-caption);font-weight:var(--weight-medium)}
.integration input,.integration select{width:100%;min-width:0;min-height:var(--control-size);border:var(--stroke) solid var(--input);border-radius:var(--radius-control);background:var(--secondary);color:var(--foreground);padding:var(--space-2) var(--space-3);font:var(--type-body)/var(--line-body) var(--font-body)}
...
function mountIntegrationForm(root,setup,access){
  ...
  let status=null;
  function link(url,label){const a=integrationNode("a",label);try{const u=new URL(url);if(u.protocol!=="https:")return integrationNode("span",label);a.href=u.href}catch{return integrationNode("span",label)}a.target="_blank";a.rel="noopener noreferrer";if(typeof access.openLink==="function")a.addEventListener("click",e=>{e.preventDefault();Promise.resolve(access.openLink(a.href)).catch(()=>{if(status)status.textContent="The host could not open this reference. Open the provider dashboard in your browser."})});return a}
  const inputs=[];
  for(const f of setup.fields){
    const wrap=integrationNode("div"),label=integrationNode("label"),row=integrationNode("div"),input=integrationNode("input");
    label.append(document.createTextNode(f.label+(f.required&&!f.stored?" *":"")));
    for(const item of Array.isArray(f.links)?f.links:[]){const source=link(item.url,item.label||"Official source");source.className="field-source";label.append(source)}
