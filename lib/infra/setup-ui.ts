...
.integration .setup-grid{display:grid;grid-template-columns:minmax(0,1fr) minmax(0,1fr);gap:var(--space-5)}.integration label{display:block;font-weight:var(--weight-strong);margin-bottom:var(--space-1)}
...
.integration details{border:var(--stroke) solid var(--border);border-radius:var(--radius-panel);padding:var(--space-3);background:var(--settings-card);margin:var(--space-3) 0}.integration summary{cursor:pointer;font-weight:var(--weight-strong)}.integration li{margin:var(--space-2) 0}.integration a{color:var(--primary);text-underline-offset:var(--space-1);overflow-wrap:anywhere}.integration .setup-status{min-height:var(--control-size);overflow-wrap:anywhere}.integration button:disabled{cursor:not-allowed;opacity:var(--settings-disabled-opacity)}
