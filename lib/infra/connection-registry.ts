...
export function connectionMethods(provider:string, source:ConnectionSource) {
  ...
  if(source==="direct") return setupMethods(p.id).map(m=>({id:m.id as string,label:m.label,scope:provider==="convex-cloud"?(m.id==="deployment"?"deployment":"account"):provider==="composio"?m.id:provider==="dokploy"?"server":provider==="hostinger"&&m.id==="mail"?"mail-order":"account",fields:setupFields(p.id,m.id),guidance:setupGuidance(p.id,m.id)}));
