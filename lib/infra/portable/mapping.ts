...
export const FIELD_MAP:Record<string,Record<string,string>>={
  ...
  hostinger:{HOSTINGER_API_TOKEN:'apiToken'},
...
export function wireMethod(provider:string,source:string,method:string){
  ...
  return provider==='composio'?(method==='organization'?'organization-token':'project-api-key'):provider==='convex-cloud'?(method==='deployment'?'deployment-key':'personal-access-token'):DIRECT[provider]??method;
}
