...
async function seal(payload,passphrase){
  ...
  const plain=Buffer.from(JSON.stringify(payload));try{const cipher=crypto.createCipheriv('aes-256-gcm',key,iv,{authTagLength:16});cipher.setAAD(AAD);const data=Buffer.concat([cipher.update(plain),cipher.final()]);
...
async function open(input,passphrase){
  ...
  try{const decipher=crypto.createDecipheriv('aes-256-gcm',key,iv,{authTagLength:16});decipher.setAAD(AAD);decipher.setAuthTag(tag);try{plain=Buffer.concat([decipher.update(data),decipher.final()])}catch{fail('wrong_passphrase_or_tampered_bundle')}
