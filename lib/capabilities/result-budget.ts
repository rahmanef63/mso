export const DEFAULT_CAPABILITY_RESULT_BYTES = 32 * 1024;
export const MAX_CAPABILITY_RESULT_BYTES = 128 * 1024;

function clampBudget(value?: number): number {
  if (!Number.isFinite(value)) return DEFAULT_CAPABILITY_RESULT_BYTES;
  return Math.max(4 * 1024, Math.min(MAX_CAPABILITY_RESULT_BYTES, Math.trunc(value!)));
}
...
export type CapabilityResultBudget = {
  maxTextBytes?: number;
  overflowHint?: string;
};
 ...
 */
export function boundedResultText(result: unknown, policy?: CapabilityResultBudget): string {
  let raw: string;
  try {
