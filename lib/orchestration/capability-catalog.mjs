const ROUTES = [
  { id: "agent-bootstrap", priority: 108, patterns: [
    /\b(mso-agent-bootstrap|agent bootstrap|mcp bootstrap|orient(?:ation)? sequence|first tools? to call|how (?:does|do i) (?:mso|this mcp) work|cara pakai mso(?:\s+mcp)?)\b/i,
    /\b(capability parity|maximal mso capabilities)\b/i,
  ], tools: ["skills_search", "skills_read"], budget: 10_000 },
  { id: "automation-flow", priority: 105, patterns: [/\b(automation flow|workflow graph|workflow|flow_catalog|flow_run|mso flow|reusable flow|alur otomatis|otomasi provider|n8n)\b/i], tools: ["workflow_graph", "flow_catalog", "flow_run", "flow_status", "flow_manage", "integration_query"], budget: 12_000 },
  { id: "project-mcp-add", priority: 105, patterns: [/\b(add|connect|tambah|pasang)\b.{0,30}\bmcp\b/i], tools: ["project_mcp_manage", "project_mcp_tools", "integration_query", "integration_setup_open"], budget: 10_000 },
  { id: "project-asset", priority: 105, patterns: [/\b(attach|save|kirim|simpan)\b.{0,40}\b(asset|image|gambar)\b.{0,30}\b(project|proyek)\b/i], tools: ["session_artifacts", "project_asset_attach"], budget: 8_000 },
  ...
  { id: "project-manual-test", priority: 95, patterns: [
    /\b(manual test|user test|i tested|i tried|still (?:broken|frozen|crashing|failing)|sudah (?:tes|test|uji)|hasil (?:tes|test|uji)|masih (?:rusak|freeze|crash|gagal))\b/i,
    /\b(now|sekarang|udah|sudah)\b.{0,24}\b(fixed|pass(?:ed)?|working|works|berhasil|normal)\b/i,
  ], tools: ["project_memory_search", "project_memory_upsert"], budget: 12_000 },
  ...
  { id: "command-verification", priority: 92, patterns: [
    /\b(run|execute|jalankan)\b.{0,24}\b(node|bun|npm|pnpm|python(?:3)?|pytest)\b/i,
    /\b(run|execute|jalankan)\b.{0,80}(?:~\/|\/(?:home|tmp|var|srv|opt)\/)[^\s,;]+\.(?:m?js|cjs|ts|py)\b/i,
  ], tools: ["exec_run"], budget: 10_000 },
  ...
  { id: "project-mcp", priority: 96, patterns: [/\b(project mcp|mcp server|mcp tool|\.mcp\.json|project connector)\b/i], tools: ["project_capabilities", "project_mcp_tools", "project_mcp_call"], budget: 10_000 },
  ...
  { id: "browser", priority: 85, patterns: [/\b(camoufox|browser|firefox|vnc)\b/i], tools: ["skills_search", "skills_read", "browser_status", "browser_power"], budget: 10_000 },
  ...
  { id: "local-agent", priority: 100, patterns: [/\b(local|same[- ]host|session)\b.*\b(agent|delegate|message|inbox|request|reply)\b/i, /\blocal_agent_/i], tools: ["local_agents_list", "local_agent_inbox", "local_agent_standby", "local_agent_message_send", "local_agent_reply", "local_agent_request_wait", "local_agent_request"], budget: 12_000 },
  ...
  { id: "repo-change", priority: 55, patterns: [
    /\b(implement|refactor|fix|update|change|migrate|add|remove|cleanup|audit|debug|diagnose|rollback|terapkan|implementasi|perbaiki|ubah|tambahkan|hapus|rapikan)\b.*\b(repo|repository|project|codebase|runtime|architecture|feature|system|proyek|kode|arsitektur|fitur|sistem)\b/i,
    /\b(repo|repository|project|codebase|proyek)\b.{0,120}\b(debug|diagnose|fix|patch|migrate|rollback|test|verify|perbaiki|uji)\b/i,
  ], tools: ["workflow_start"], budget: 16_000 },
];
