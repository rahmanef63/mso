...
describe("MCP integration setup privacy", () => {
  ...
  it("requires write scope, accepts no credential parameters, and uses the existing Page", async () => {
    ...
    expect((tool.meta?.ui as { resourceUri?: string }).resourceUri).toMatch(/^ui:\/\/mso\/page-v16\.html$/);
    expect(tool.meta?.["ui/resourceUri"]).toBe((tool.meta?.ui as { resourceUri?: string }).resourceUri);
