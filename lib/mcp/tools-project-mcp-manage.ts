...
export const PROJECT_MCP_MANAGE_TOOLS: McpTool[] = [{
  ...
  chatgptDescription: "Manage installed MCPs: project=@host selects this VPS, otherwise choose one exact project. Inspect first; then discover via project_mcp_tools and call via project_mcp_call. No inherited bindings.",
  meta: { ui: { visibility: ["model", "app"] }, "openai/widgetAccessible": true },
  annotations: { readOnlyHint: false, destructiveHint: true, openWorldHint: false },
  limit: { key: "project.mcp.manage", max: 20, windowMs: 60_000, actionLimits: { inspect: { max: 120, windowMs: 60_000 } } },
  audit: { action: "fs.write", targetArg: "project" },
  inputSchema: S({
    project: { type: "string", description: "Exact project id/path/name, or @host for the MSO host installation scope." },
    ...
    plugin: { type: "string", enum: ["si-coder", "batonly"], description: "Reviewed project plugin identity. Installation is explicit-target only; no default activation or project inheritance." },
