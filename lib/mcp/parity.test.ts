...
const MCP_ONLY: Record<string, string> = {
  ...
  "agent.session.action.resolve": "stable semantic action resolution belongs to the same external durable AgentSession ledger and preserves owner isolation; Alfa browser threads keep their separate conversation identity",
  ...
  "local.agent.standby": "durable standby is bound to an external/CLI AgentSession plus exact active workflow; Alfa browser threads do not share that durable session/workflow identity, while Cockpit only observes its safe projected status",
