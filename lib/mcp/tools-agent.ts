...
export const AGENT_TOOLS: McpTool[] = [
  ...
  },
  {
    ...
    run: async (a, context) => {
      ...
      const flow = semanticSessionFlow(target.events, Math.max(1, Math.min(120, Number(a.limit) || 120)), target.cwd, target.eventSeqBase);
  ...
  {
    name: "agent_session_action_resolve",
    description: "Resolve one stable human session action reference such as S3.A4 (or E217) to its exact sanitized semantic step/action receipt. The lookup is owner-scoped and never exposes the internal session id.",
    scope: "read", annotations: { readOnlyHint: true, idempotentHint: true },
    inputSchema: S({
      session_ref: { type: "string", description: "Human session label (agent-context), @name, title, or legacy exact id." },
