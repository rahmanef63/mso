...
export const LEGACY_LIST_V2_URI = "ui://mso/list-v2.html";
export const LEGACY_LIST_V1_URI = "ui://mso/list-v1.html";
export const LEGACY_BLOCK_V4_URI = "ui://mso/block-v4.html";
export const LEGACY_BLOCK_V3_URI = "ui://mso/block-v3.html";
export const LEGACY_BLOCK_V2_URI = "ui://mso/block-v2.html";
export const LEGACY_PAGE_V15_URI = "ui://mso/page-v15.html";
...
export const LEGACY_NATIVE_UI_PROBE_V1_URI = "ui://mso/native-probe-v1.html";
...
export async function listUiResources(): Promise<Array<{ uri: string; name: string; description: string; mimeType: string; _meta: Record<string, unknown> }>> {
  const page = await msoPageResource();
  return [MSO_LIST_RESOURCE, MSO_BLOCK_RESOURCE, page, MSO_NATIVE_UI_PROBE_RESOURCE].map(
    ({ uri, name, description, mimeType, _meta }) => ({ uri, name, description, mimeType, _meta }),
  );
}
...
export async function readUiResource(uri: string): Promise<McpUiResource | undefined> {
  ...
  if (uri === LEGACY_LIST_V2_URI || uri === LEGACY_LIST_V1_URI) return { ...MSO_LIST_RESOURCE, uri };
  ...
  if (uri === LEGACY_NATIVE_UI_PROBE_V1_URI) return { ...MSO_NATIVE_UI_PROBE_RESOURCE, uri };
  if (uri === LEGACY_BLOCK_V4_URI || uri === LEGACY_BLOCK_V3_URI || uri === LEGACY_BLOCK_V2_URI || uri === LEGACY_BLOCK_V1_URI || uri === LEGACY_WORKFLOW_PROGRESS_URI) return { ...MSO_BLOCK_RESOURCE, uri };
