...
import { uploadOneGuarded, type UploadConflictPolicy } from "@/lib/host/fs-api";
...
const EXTENSION_MIME = new Map([
  ...
  [".jpeg", "image/jpeg"], [".json", "application/json"], [".zip", "application/zip"],
]);
...
function configuredAzureHosts(): Set<string> {
  ...
  for (const raw of (process.env.OS_MCP_OPENAI_FILE_HOSTS || "").split(",")) {
    ...
    if (!host) continue;
    if (!/^[a-z0-9-]+\.blob\.core\.windows\.net$/.test(host)) throw new HostError("OS_MCP_OPENAI_FILE_HOSTS contains an invalid Azure Blob hostname");
    hosts.add(host);
  }
  return hosts;
}
function trustedDownloadUrl(raw: string, allowChatGptAzureFamily = false): URL {
  ...
  if (!openAiContentHost && !trustedChatGptAzure && !configuredAzureHosts().has(host)) throw new HostError(`file.download_url host is not allowed: ${host}`);
  return url;
}
...
async function readBoundedBody(response: Response): Promise<Buffer> {
  ...
  if (rawLength !== null) {
    ...
    if (!Number.isFinite(declared) || declared < 0 || declared > MAX_FILE_BYTES) throw new HostError("file exceeds the 20 MiB MCP import limit");
  }
...
function declaredMime(file: OpenAiProvidedFile): string {
  ...
  const inferred = EXTENSION_MIME.get(path.extname(file.file_name || file.name || "").toLowerCase());
...
async function validRaster(data: Buffer, mimeType: string): Promise<boolean> {
  if (!hasImageSignature(data, mimeType)) return false;
  try {
    const sharp = (await import("sharp")).default;
    const decoded = await sharp(data, { failOn: "error", limitInputPixels: MAX_IMAGE_PIXELS, sequentialRead: true }).raw().toBuffer({ resolveWithObject: true });
    return decoded.info.width > 0 && decoded.info.height > 0 && decoded.data.byteLength > 0;
  } catch { return false; }
}
function validZip(data: Buffer): boolean {
  if (data.length < 22) return false;
  const eocd = data.lastIndexOf(Buffer.from([0x50,0x4b,0x05,0x06]));
  if (eocd < Math.max(0, data.length - 65_557) || eocd + 22 > data.length) return false;
  return eocd + 22 + data.readUInt16LE(eocd + 20) === data.length;
}
async function validateBody(data: Buffer, mimeType: string): Promise<boolean> {
  if (mimeType.startsWith("image/")) return validRaster(data, mimeType);
  if (mimeType === "application/zip") return validZip(data);
  if (mimeType === "application/json") {
    try { JSON.parse(new TextDecoder("utf-8", { fatal: true }).decode(data)); return true; } catch { return false; }
  }
  return false;
}
function safeFilename(input: string | undefined, fallback: string, mimeType: string): string {
  ...
  if (EXTENSION_MIME.get(path.extname(value).toLowerCase()) !== mimeType) throw new HostError("filename extension does not match the validated file type");
  return value;
}

export async function importOpenAiProvidedFile(opts: {
  file: unknown; dest: string; filename?: string; conflict?: UploadConflictPolicy; expectedSha256?: string; allowChatGptAzureFamily?: boolean;
}) {
