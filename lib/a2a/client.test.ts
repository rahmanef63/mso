...
import { a2aAgentCardUrl, cancelA2ATask, discoverA2AAgent, getA2ATask, MAX_A2A_MESSAGE_BYTES, MAX_A2A_RESPONSE_BYTES, sendA2AMessage } from "./client";
...
it("uses v1 HTTP+JSON operation paths for send/get/cancel including tenant", async () => {
  ...
  expect(calls[2].url).toBe("https://agent.example.test/a2a/tasks/t-1?tenant=research&historyLength=4");
  expect(calls[3].url).toBe("https://agent.example.test/a2a/tasks/t-1:cancel");
  expect(JSON.parse(String(calls[3].init?.body))).toEqual({ tenant: "research" });
});
...
it("rejects insecure/private targets before any A2A network call", () => {
  ...
  expect(() => a2aAgentCardUrl("https://127.0.0.1:9999")).toThrow(/host not allowed/i);
});

it("rejects peers that advertise only pre-v1 interfaces", async () => {
  const legacy = { ...card(), supportedInterfaces: [{ url: "https://agent.example.test/a2a", protocolBinding: "JSONRPC", protocolVersion: "0.3" }] };
  await expect(discoverA2AAgent("https://agent.example.test", async () => response(legacy))).rejects.toThrow(/no supported v1/i);
});

it("bounds Agent Card/remote response bytes and outbound message bytes", async () => {
  const oversized = new Response("{}", { status: 200, headers: { "content-length": String(MAX_A2A_RESPONSE_BYTES + 1) } });
  await expect(discoverA2AAgent("https://agent.example.test", async () => oversized)).rejects.toThrow(/response exceeds/);
  const agent = await discoverA2AAgent("https://agent.example.test", async () => response(card()));
  await expect(sendA2AMessage(agent, "x".repeat(MAX_A2A_MESSAGE_BYTES + 1), {}, async () => response({}))).rejects.toThrow(/message exceeds/);
});
