...
The MCP server also advertises `capabilities.extensions["io.modelcontextprotocol/ui"]` with the MCP App HTML MIME in legacy `initialize` and modern `server/discover`. Native presentation is therefore negotiated explicitly with a host that advertises the same extension; an orchestration/function wrapper may still call the tool while omitting that host-owned presentation context.
...
Fresh 3 also advertises the bounded `io.modelcontextprotocol/skills` extension. `skills/list`, `skills/get`, and `resources/read` publish a submission-time snapshot of exactly five general official MSO skills: `mso-agent-bootstrap`, `mso`, `mso-repo-work`, `mso-service-debug`, and `mso-deploy`. `mso-agent-bootstrap` is first: it is the first-call orientation sequence for MCP agents. Implementer skill `mso-mcp-feature-engineering` stays in the live `skills_search` catalog. Each published skill is now a reviewed bundle containing `SKILL.md`, `contract.yaml`, and `agents/openai.yaml` (plus any skill-specific resources). `contract.yaml` carries structured routing/lifecycle/safety metadata; `agents/openai.yaml` carries concise OpenAI display/default-prompt metadata. Publication fails closed when either structured file is missing or invalid. The manifest still includes complete resources plus `sha256:` digests, refuses symlinks/path normalization conflicts/oversize files, and never publishes operator, project, verified-third-party, or untrusted skill roots. The ordinary `skills_search/list/read` actions remain the live runtime catalog for the rest.
...
**Staging is preparation, not installation.** It does not install, publish, refresh, or select the app in ChatGPT. `bun run plugin:doctor-private` reports only redacted staging/install-state facts for the machine on which it runs. A local marketplace/plugin cache belongs to that client machine; staging on a remote VPS does not make a ChatGPT web or desktop conversation use that package.

For native MCP Apps acceptance, start a new conversation through the registered MCP App/direct app context (or an actually installed plugin package that references it). A generic function, dynamic-tool, Code Mode, or plugin orchestration wrapper may preserve the tool schema while unwrapping the MCP result and omitting the host-owned app resource context. If `render_mso_block` returns only structured data in such a wrapper, that is not evidence that the server resource, HTML, CSP, or bridge is broken. Verify that the host invocation retains the app resource binding before changing renderer code. Only a visually mounted component and a real component action round-trip count as native UI acceptance.
