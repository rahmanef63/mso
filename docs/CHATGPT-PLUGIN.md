...
`render_mso_page` and `integration_setup_open` bind Page through the standard MCP Apps `ui.resourceUri` field only. Do not mirror `openai/outputTemplate` onto Page tools: the current ChatGPT host already supports the standard binding, and dual Page bindings can mount the same UI twice. Page v10 therefore uses one modern binding and keeps earlier Page URIs only as non-advertised read aliases.

Reviewed app previews enter in **inline** mode. The widget derives an inline height cap of 48% from the host-provided `maxHeight`, so an ordinary chat preview stays below half of the available host area and scrolls internally when needed. Fullscreen and PiP remain explicit user actions and are not constrained by the inline cap.
