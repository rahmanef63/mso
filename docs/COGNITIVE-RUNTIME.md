...
- **Markdown compatibility projection** — `USER.md` / `MEMORY.md` remain the resolved materialized view consumed by older CLI/session code. Existing unstructured legacy documents remain bounded to 64 KiB when imported; once a typed ledger exists, its materialized compatibility projection may grow up to the same 2 MiB safety envelope as the ledger so `remember`/`forget` cannot become impossible merely because resolved Markdown crossed the legacy import limit. Existing installations are read unchanged; the first structured mutation seeds the ledger from legacy `## key` sections instead of deleting or rewriting history.
