...
- Fresh 3 current MCP contract: server `1.9.3` / toolset `2026.09.04.4`; ChatGPT receives **64 transport tools = 63 model/operator actions (34 read / 17 write / 12 exec) + app-only `workflow_status`**, while the full transport catalog is 88 tools / 87 model tools. Every ChatGPT action advertises `outputSchema`, the dedicated MCP Apps widget origin is `https://mso-ui.rahmanef.com`, and five official digest-verified Skills are exposed through the MCP Skills/resource extension. Exactly two MCP App resources are published: dedicated `ui://mso/workflow-progress-v2.html` plus universal `ui://mso/surface-v4.html`. `project_get`, `project_diff`, and `vps_status` are pure data tools with no UI template; Surface native views refresh them through MCP Apps `tools/call`. The Surface accepts no raw HTML/arbitrary URL; its only frame domain is `https://game.rahmanef.com`, and every Play Together iframe URL must remain under `/embed`. The normal Play Together shell stays anti-frame, while the dedicated embed namespace allows only `mso-ui.rahmanef.com`. Other MSO server scopes are not copied into this catalog.
- MCP App navigation acceptance: a ChatGPT-opened root URL carrying `redirectUrl=https://chatgpt.com/c/...` must 307 to `/assistant/mcp`, then authenticated browser E2E must show the Alfa `MCP Activity` view. The dedicated workflow card targets `/assistant/mcp`; VPS/project/diff are now native routes inside universal Surface v4 rather than independent widgets. Surface v4 is a cache-busting resource URI change, so ChatGPT must re-scan Fresh 3 after deployment.
