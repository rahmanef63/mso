...
- MCP App navigation acceptance: a ChatGPT-opened root URL carrying `redirectUrl=https://chatgpt.com/c/...` must 307 to `/assistant/mcp`, then authenticated browser E2E must show the Alfa `MCP Activity` view. The dedicated workflow card targets `/assistant/mcp`; VPS/project/diff are native routes inside universal Surface v5 rather than independent widgets. Surface v5 plus workflow-progress-v3 are cache-busting resource URI changes, so ChatGPT must re-scan Fresh 3 after deployment.
