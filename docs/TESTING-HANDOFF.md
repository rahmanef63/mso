...
- Fresh 3 current MCP contract: server `1.9.2` / toolset `2026.09.04.3`; ChatGPT receives **64 transport tools = 63 model/operator actions (34 read / 17 write / 12 exec) + app-only `workflow_status`**, while the full transport catalog is 88 tools / 87 model tools. Every ChatGPT action advertises `outputSchema`, the dedicated MCP Apps widget origin is `https://mso-ui.rahmanef.com`, and five official digest-verified Skills are exposed through the MCP Skills/resource extension. Five MCP Apps are published: the four existing v2 operator cards plus `ui://mso/surface-v3.html`. The Surface accepts no raw HTML/arbitrary URL; its only frame domain is `https://game.rahmanef.com`, and every Play Together iframe URL must remain under `/embed`. The normal Play Together shell stays anti-frame, while the dedicated embed namespace allows only `mso-ui.rahmanef.com`. Other MSO server scopes are not copied into this catalog.
- MCP App navigation acceptance: a ChatGPT-opened root URL carrying `redirectUrl=https://chatgpt.com/c/...` must 307 to `/assistant/mcp`, then authenticated browser E2E must show the Alfa `MCP Activity` view. Contextual widget targets are workflow→`/assistant/mcp`, VPS→`/monitor`, project→`/files`, diff→`/code`. This changes no MCP tool/schema/resource URI, so existing Fresh 3 registrations remain compatible.
