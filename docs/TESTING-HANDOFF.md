...
- Fresh 3 MCP presentation remains two canonical resources (Block + Page). The Page widget origin is now deployment-derived from `OS_MCP_UI_ORIGIN` / `OS_PUBLIC_ORIGIN`; portable source carries no operator domain. External Page apps default to none and are opt-in through validated per-installation `MSO_SURFACE_APPS_JSON`. `workflow_start` remains headless; Block has no frame domains and cannot execute a mutation directly.
