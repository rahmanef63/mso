...
- Fresh 3 current MCP contract: server `1.9.1` / toolset `2026.09.04.2`; ChatGPT receives **64 transport tools = 63 model/operator actions (34 read / 17 write / 12 exec) + app-only `workflow_status`**, while the full transport catalog is 88 tools / 87 model tools. Every ChatGPT action advertises `outputSchema`, the dedicated MCP Apps widget origin is `https://mso-ui.rahmanef.com`, and five official digest-verified Skills are exposed through the MCP Skills/resource extension. Five MCP Apps are published: the four existing v2 operator cards plus corrected `ui://mso/surface-v2.html`. The Surface accepts no raw HTML/arbitrary URL and currently has **zero third-party frame domains**. Its VPSKU target is Play Together (`game.rahmanef.com`) via the remote-browser seam while that project retains `frame-ancestors 'self'`. Other MSO server scopes are not copied into this catalog.
