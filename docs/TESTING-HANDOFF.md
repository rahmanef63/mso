...
- Fresh 3 MCP presentation now has three canonical resources (List + Block + Page). The Page widget origin is now deployment-derived from `OS_MCP_UI_ORIGIN` / `OS_PUBLIC_ORIGIN`; portable source carries no operator domain. External Page apps default to none and are opt-in through validated per-installation `MSO_SURFACE_APPS_JSON`. `workflow_start` remains headless; Block has no frame domains and cannot execute a mutation directly.
- MCP App acceptance: starting a workflow must produce structured workflow identity without opening an App. A Block must render bounded validation/CRUD data and return a user-approved follow-up when its action is pressed; it must not call a mutation itself. A Page must render native `/monitor`/`/project`/`/diff` views through `tools/call`; any external app must come only from the current installation's validated catalog. Compatibility URIs remain non-advertised aliases.
