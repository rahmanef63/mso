...
Fresh installs write `OS_MCP_ENABLED=1`, so `/mcp` and OAuth discovery are available for connector
setup by default. They are still protected: OAuth consent and a bearer are required for operational
calls. If an administrator unsets/disables MCP, `/mcp` and both OAuth discovery documents return 404;
demo mode also forces MCP off. Existing installs keep their current `.env.local` unchanged on update.
...
## Fresh-install defaults and opt-down

```bash
# .env.local generated by a fresh install
OS_MCP_ENABLED=1
OS_MCP_MAX_SCOPE=exec    # read | write | exec
```

`exec` is a powerful ceiling: if that bearer leaks, the holder can run remote commands as the MSO
service user. Lower `OS_MCP_MAX_SCOPE` to `write` or `read` before connecting clients that do not need
shell execution. Existing `.env.local` files are preserved during update rather than rewritten to
these new defaults.

For a public connector use a stable HTTPS origin, normally a named Cloudflare Tunnel backed by
`http://127.0.0.1:4005`; set it with `mso gateway domain set https://mso.example.com`. Never use a
...
Picked per token, on the consent screen, capped by `OS_MCP_MAX_SCOPE`. The fresh-install ceiling is
`exec`, so the highest tier is available/preselected unless the administrator lowers it. The OAuth
discovery metadata advertises `read`, `write`, `exec`, and `offline_access`; lowering the ceiling does
not remove the protocol vocabulary, it prevents consent from granting a higher operational tier.
...
Local same-owner session collaboration is intentionally separate from public A2A. `local_agents_list` reports presence/foreground receiver state (`consumerConnected` / `consumerCount`) separately from durable standby state (`standbyArmed`, `standbyState`, exact workflow, queue/actionable metadata). `local_agent_message_send` + `local_agent_reply` provide durable correlated mailbox semantics. `local_agent_inbox(wait_ms=...)` remains the bounded 0–20s **foreground** receiver and does not become a daemon.

`local_agent_standby` is the exec-scoped server-native path. `listen` validates the conversation-bound session plus exact active workflow, persists the armed record, subscribes a standby listener that does not increment `consumerConnected`, closes the subscribe/read race against the durable mailbox, and returns immediately. Executable requests carry structured authorization metadata; write-only requests, `notify`, and `reply` cannot wake a model worker. A durable claim + session lease enforces one active command per standby session, later requests remain queued, workflow finish/cancel disarms the record, and process restart reconciliation reinstalls listeners then drains eligible durable mail. Execution reuses the existing bounded durable-session/A2A task engine rather than introducing a second agent runtime.
...
entrypoint is `/integrations`; The current Page retains previous resource aliases. Page-bound tools advertise only the standard MCP Apps `ui.resourceUri`; the legacy `openai/outputTemplate` Page alias is intentionally omitted to prevent duplicate host mounts. Inline Page fills the host-declared fixed or maximum container height (legacy maxHeight, then 680 px fallback); fullscreen/PiP require explicit user action. Standard size notifications keep flexible host frames synchronized.
