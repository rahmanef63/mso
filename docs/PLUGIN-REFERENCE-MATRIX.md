...
| OpenAI app binding accepts registered apps/connectors with required-or-optional state | No decisive evidence | Required registered-app binding | Strong: exact `connector_*` binding with `required:false`; public plugin guidance also recognizes connector/template bindings | pending | pending | pending | pending | pending | pending | pending | pending | pending | Checker previously accepted only `asdk_app_*` + `required:true` | App-binding compatibility validator | `NEW_GENERALIZABLE` — implemented without changing authorization |
