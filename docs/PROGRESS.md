...
- terminal title art now uses a stable two-space gutter so the blue MSO icon is not flush against the terminal edge;
- the first MSO folder-title row now carries the same internal two-space inset as the folder body, and the terminal composer now opens a Hermes-style slash dropdown immediately on `/`, with live filtering, fixed 8-row viewport, ↑/↓ navigation, Enter/Tab selection, Esc dismissal, command metadata, and trusted current-project/global skill recommendations;
...
- CLI version for this interaction milestone is **1.5.3**. `docs/INFRASTRUCTURE-PROVIDERS.md`, architecture/install/MCP/ChatGPT connector docs, generated CLI reference and slice catalog are synchronized.
