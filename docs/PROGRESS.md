...
Optimizer v1 deliberately separates host authority from probabilistic judgment. MSO derives the eligible transformations, preserves capability scope/auth/confirmation/audit boundaries, and can run fully deterministic. Safe presentation grouping collapses consecutive action/context nodes through existing `metadata.customNodes` without changing execution. Structural compaction is narrower: three or more consecutive calls to the same read-only MSO tool can become one sequential Loop node carrying the exact already-prepared arguments, and this requires explicit review opt-in because the output shape changes. Write/exec tools, branched topology and referenced intermediate outputs are not compacted by this pass.

Jev is optional and acts only as a decision kernel. A named direct Project MCP Integration keeps its endpoint/bearer private, applies an exact optional tool allowlist, and exposes bounded `mcp.tools.list` / `mcp.tool` execution through the existing Integration authority. The adapter discovers the remote tool schema first and supports both the bounded-alternative `jev_decide` contract and typed `state/questions` evaluators. Jev receives a compact workflow/candidate summary rather than graph credentials or new executable calls; it cannot invent arguments or bypass MSO policy. Unsupported/unavailable Jev falls back deterministically, and Jev mode reports decision-shadow agreement against the deterministic baseline without double-running workflows or duplicating side effects.

The native Workflows UI adds Optimize with Analyze / Create optimized draft, provider/risk/probability summaries, node/edge reduction, fallback evidence and shadow agreement. Targeted optimizer/integration/MCP tests, Workflow Graph regressions, integration-model regressions, TypeScript, ESLint, zero-cycle architecture checks, docs/contrast/skill/audit gates, isolated production build, bundle budget and the full desktop/mobile release E2E all pass.
...
The remaining MSO compatibility gap was in registration/discovery normalization. MSO already emitted canonical _meta.ui.resourceUri, but unlike the official ext-apps helper it did not mirror that URI into the compatibility _meta["ui/resourceUri"] field, and resources/list discarded each resource's _meta.ui even though the helper keeps listing-level UI metadata for host indexing. All five resource-bound tools now publish both URI forms while keeping _meta.ui.resourceUri authoritative; resources/list preserves bounded resource _meta; Block retains its older openai/outputTemplate fallback and the other tools do not gain it.

Because the failing ChatGPT widget URLs are cache/index keys, all four canonical resources were also advanced to fresh URIs: native-probe-v2, block-v5, list-v3 and page-v16. The immediately previous canonical URIs remain read-only aliases so already-scanned clients do not break, but are not advertised. Regression now locks flat/nested descriptor parity, listing-level resource metadata and previous-URI aliases. MCP server/toolset identity advances to 1.15.3 / 2026.09.22.2. Final visual acceptance still requires a fresh ChatGPT Scan Tools snapshot and a new app-selected conversation; success means the internal widget request no longer returns 404, the component is visibly mounted, and a real component action round-trip succeeds.
...
## 2026-09-22 — Split n8n from Workflows and make Organization/Workflow usable at scale

The Organization project map had become visually unreadable on dense units because the initial canvas fit tried to show the entire graph at once. Organization is now directory-first: the owner sees searchable unit cards and operational counts, opens a unit into explicit Projects/Seats surfaces, and can switch to relationship maps only when topology matters. Seat browsing follows the same directory-first rule. Shared graph fitting now supports a representative readable initial cluster while preserving `F` as full-graph fit, and the Organization sidebar exposes direct Workflows, Integrations and System links without changing capability authority.

External n8n is no longer a provider tab inside native Workflows. It is a first-class `/n8n` app/route with its own reviewed surface placement, cookie-isolated authentication boundary, responsive embed/fallback, and direct links back to native Workflows, Organization and Integrations. Legacy reviewed `workflows` placement remains readable during migration, while new registrations use `n8n`. Role-filtered or unavailable apps now show a neutral unavailable-state instead of the misleading `Unknown app` message.

Native Workflows gained the missing core management pieces needed for practical n8n-style self-hosted parity: persistent owner-private data tables plus a `data_table` node; execution stop/retry/delete controls; portable `.mso-workflow.json` import/export; clearer Builder/Current run/Executions/Versions/Variables/Data/Directory panels; a denser workflow library; and toolbar overflow behavior that keeps wide action sets from pushing the canvas beyond its pane. Execution retry retains runtime input only in the private receipt, strips it from public receipts, requires the current graph revision to match, and stop remains cooperative at bounded node boundaries. Credentials remain owned by Integrations, Organization remains units/seats/projects/routing context, and Workflow remains automation topology/execution. `docs/N8N-PARITY.md` records the domain map rather than claiming pixel/API parity or copying n8n billing/marketplace administration.

Verification on the isolated candidate: TypeScript, ESLint, architecture/docs/contrast/skills/instance-literal checks, the full coverage gate, production build, and the complete release E2E all pass, including dedicated n8n, Organization flow, custom graph, desktop/mobile/landscape reflow, data-table/portability/execution-control regressions, and no browser errors.
...
This release adds `Repeat Until` as a first-class Flow node without weakening the DAG safety boundary. A Repeat node references another saved Workflow Graph by `workflowId`, supplies its normal input plus `input.repeat.iteration` and the previous iteration result, reads the child workflow's explicit Output node, and evaluates a configurable result path. Exact JSON comparison is available through `equals`; omitting it uses truthiness. A match follows the `done` port, while reaching the configured iteration limit follows `exhausted`. Child failures still use the normal workflow error/retry semantics.

The loop is intentionally bounded rather than an unrestricted visual back-edge: `maxIterations` is 1–50, inter-iteration delay is bounded, total repeat duration is capped below the parent run timeout, cross-workflow ancestry still rejects recursion, and every child execution receives its own persisted run receipt. The parent stores compact iteration metadata and final output instead of duplicating full child receipts. Existing acyclic graphs, item-loop behavior, Subflow, schedules/webhooks, and Session workflow drafts remain compatible.

The Workflow inspector exposes the saved workflow selector, input bindings, condition path/value, iteration/delay/duration bounds, and `done` / `exhausted` ports. The Sessions view now makes the reuse path explicit: a reviewed “Save session as workflow draft” result can become the body of a Subflow or Repeat Until node. AI-assisted workflow drafting has the same bounded Repeat contract. Regression proves a saved child workflow matching on iteration three, the exhausted path after two iterations, Session draft behavior, graph layout, and existing flow controls. Full `bun run verify` and the official isolated production build/release E2E both pass.
...
This release adds `local_agent_standby` as an explicit exec-scoped control-plane primitive bound to the exact conversation session and active workflow. `listen` persists an owner-private standby record and returns immediately. A separate standby event channel rehydrates after process restart, rereads the durable mailbox to close subscribe/read races, atomically claims one authorized request, executes it through the existing durable-session/A2A task engine, emits the correlated reply, and returns the worker to `waiting`. Additional requests remain queued and drain sequentially. `stop`, `workflow_finish`, and `workflow_cancel` disarm the matching record; dispatch revalidates the workflow again immediately before model/tool execution.

Authorization and observability remain fail-closed. `consumerConnected` still means a real foreground receiver and standby listeners never increment it. Directory/UI state exposes `standbyArmed`, standby state/workflow, actionable state, queue count, and current message separately. Only structured correlated requests persisted by an exec-scoped caller are executable; write-only callers may enqueue but cannot grant execution, while notify/reply mail never starts a worker. Standby workers reuse the original workflow actor internally and cannot start/finish/cancel a replacement workflow. Durable message claims plus a per-session execution lease prevent overlapping event/restart workers; after a crash MSO can recover an expired claim, but intentionally does not claim exactly-once external side effects across an abrupt process death.

The implementation keeps the existing foreground inbox path, local one-shot request path, remote A2A protocol, and timed Workflow Graph wait node unchanged. Local Agent storage/dispatcher modules were split to stay inside the architecture line-count ratchet instead of raising policy limits. Targeted release regression covers immediate arm, disconnected-but-actionable presence, correlated execute/reply/waiting, duplicate event dedupe, sequential queueing, restart reconciliation, lifecycle disarm, stale workflow blocking, notify/write-scope non-execution, same-owner isolation, foreground inbox regression, local A2A worker reuse, fixed workflow propagation, MCP route/catalog/audit authorization, and Cockpit status projection.

## 2026-09-20 — Stage MSO as a portable OpenAI Agent Plugin

Native MCP App acceptance still returned structured data when the current ChatGPT conversation reached MSO through the generic orchestration wrapper. Server descriptors/resources were already correct, but the owner-private plugin package was legacy-only and staged rather than installed. Private staging now emits the current portable Agent Plugins layout: root `plugin.json` with `extensions.com.openai.apps`, the reviewed `claude-skills/` tree projected to root `skills/`, private redacted `.app.json`, and a rewritten `.codex-plugin/plugin.json` compatibility fallback. The doctor validates both manifests, the projected skill set, owner-only modes and registered-app binding without printing the App ID, and recognizes both portable and legacy cached installs. Repository `.app.json` remains empty and no MCP credential or maintainer-specific App ID is committed. This fixes the MSO-owned packaging gap; ChatGPT installation/refresh/selection remains host-owned and must be accepted from a new app-selected conversation.
...
## 2026-09-17 — Doctor respects no-systemd and remote runtime boundaries

A live hosted/container audit reproduced a false `mso.service not active` failure while the loopback runtime, public HTTPS and authenticated CLI were healthy. Doctor now probes actual systemd manager availability before diagnosing its service and never inspects this machine's units for a remote `--base`. No-systemd hosts continue through the normal health/auth checks without suppressing real failures: health must be JSON identifying a healthy MSO service, and local probes bypass outbound proxy variables. `--fix` never creates a tunnel, adopts an external supervisor, or claims boot persistence. Added regressions for unavailable managers, inactive real services, remote bases, failed health and unrelated HTTP 200 responses; documented the hosted startup and verified-public-origin boundaries. This changes diagnosis only; no runtime, bind, provider, Node version or existing hosted Hermes/update behavior is replaced.
...
Audited the local OpenAI `plugin-management` 0.1.0 package plus its installed tool contracts. The recurring improvement is model-facing capability selection and terminology, not a reason to collapse MSO authorities: Skills remain trusted instructions, Project Plugins remain exact-project capability packages, MCP remains the tool protocol, Integrations remains the credential authority, Apps remain workspace UI, and Managed Apps remain external service runtimes. Added the living `docs/PLUGIN-REFERENCE-MATRIX.md`, made that taxonomy canonical in `docs/ARCHITECTURE.md`, and updated the main `/mso` skill to reuse existing native/project/integration capabilities before adding a new plugin. No tools, scopes, credential behavior, low-level primitives, or descriptor schemas were removed or added. Generic suggestion state, dependency graphs, platform-style permission inheritance, and universal lifecycle/postcondition engines remain cross-reference candidates rather than one-plugin abstractions.
...
The concurrent `/integrations/embed` fix is retained as a same-service alias alongside
...
The two concurrent implementations are consolidated into one `integrations` feature slice,
not two app descriptors or managers. The native window uses `/connections` and the internal
manager endpoint; the earlier `embed=shell` compatibility alias is retained with same-origin
framing only. Both standalone deny-framing and package-entrypoint spoof regression tests
remain. An explicit invalid `MSO_SC_BIN` never falls back to another user's/default CLI.
...
A required PTY test exposed an exit/data ordering race under full-suite load. Its harness now waits for both successful process exit and the final quit payload, preserving the real-terminal assertion instead of resolving a partially drained transcript. Semgrep 1.174.0 and the latest 1.176.0 both report zero findings but forty parser diagnostics on valid Bash/TypeScript/JSX; this remains a scanner coverage limitation, not evidence of complete SAST coverage. No scanner excludes or suppressions were added.

## 2026-09-09 — Zero-warning quality gate and layered MCP settings

User review found MCP confusing despite the earlier release. Live public inspection reproduced an endless MCP loading placeholder after unauthorized access. Owner setup mixed client guides, transport details, access tokens, catalog signatures and audit activity in one scroll. MCP now starts with a task index and drills into client selection then one guide. Connection details, connected-app management, audit and tools each have a separate view and explicit return path. Signed-out users get a direct Owner login preserving the MCP destination; delegated users retain the Owner boundary. Requests are bounded and failed activity never masquerades as no events. Disconnection requires an inline confirmation; active access and expired/revoked history are separated. Refresh acknowledgement explicitly remains a browser-local reminder. Settings icons use theme tokens; navigation and copy targets are larger and clipboard failures are visible. Manual OAuth copy now matches the implemented one-hour access / rotating refresh contract.

All eleven file-length warnings are resolved by responsibility-based extraction, including session creation/mutation/summary, filesystem credential/recursive policy, write/exec catalog declarations, CLI local commands/help, device locking, widgets, and focused test suites. Filesystem containment checks remain in their original sink branches. The lint release gate accepts zero warnings and the physical-line architecture ratchet is reduced. Secret tests use real disposable synthetic files; Forge Docker tests execute unconditionally with the official locally built sandbox. Four live HTTP smoke tests run as a required separate production suite inside release verification and fail on missing configuration. No tests are removed or disabled to get a green report. Browser regression coverage exercises MCP owner/public navigation, client selection, cancel/revoke, error/retry, mobile reflow and automated accessibility.

## 2026-09-09 — Audit hardening and required browser release journeys

The audit found outdated security dependencies, implicit command retargeting, historical connection checks presented as current health, and an indirect sign-in path. Patched dependency floors retain the existing stack. Explicit invalid cwd now fails before exec/job/PTY launch; successful one-shot results expose their resolved cwd. Connection doctor pins credentials and records a bounded result for that exact revision; rejected access and unavailable providers no longer display an old green Verified state. Credential rotation and later checks reject stale results.

Integrations now links directly to login with a validated same-origin return destination, refreshes cards after Verify, uses embedded provider SVG marks where available, and exposes setup instructions without two collapsed levels. Signed-out desktop entry labels mock data persistently; Settings avoids fetching owner AI configuration for anonymous/delegated visitors. Standalone heading hierarchy and verified text contrast are corrected.

The release build is followed by synthetic browser journeys for desktop, phone portrait and landscape, login/return, a local provider that changes from success to 401, state after reload, harmless file/exec operations and live device-role demotion. Public Integrations also runs automated accessibility checks. All fixture state is disposable; no production credentials are used. Unit test environment baselines and two-instance resource/CSP tests prevent host configuration from changing default-test expectations. Coverage floors follow the observed baseline; palette regressions now fail; strict dependencies are checked daily in CI. Initial validation: full `bun run verify` passed with 2,668 tests (7 environment-dependent/legacy skips), 44.02% line coverage, zero lint errors, and zero palette contrast failures. The exact committed production build passed all required browser journeys at 1363×936, 390×844 and 844×390, including automated WCAG checks on public Integrations. Strict dependency audit found no high/critical advisories. Eleven existing file-length lint warnings remain. This is synthetic-provider/browser evidence; it does not claim live third-party OAuth, physical-device/screen-reader, load or disaster-recovery coverage. The canonical ship gate repeats verification before deployment. Production screenshot review additionally exposed a mobile mode badge overlapping the Settings title; compact mobile placement and a required geometric non-overlap assertion correct that regression.

### Security scanner follow-up

Public GitHub CI passed, but separate security gates identified a legacy E2E credential flow, a missing explicit GCM tag-length option, and historical scanner false positives. Native desktop and Preview scripts now run a built app with disposable credentials/stores and explicitly seeded live test preferences; they never read deployment passwords or approved production devices. Authentication refuses redirects. AES-GCM encryption/decryption explicitly require 16-byte tags, with a regression test rejecting truncated tags before import. The history allowlist adds only reviewed commit/path/line fingerprints for synthetic Dokploy keys and DOM maxLength/hidden assignments; no broad rule/path exclusion or history rewrite is used. Chained form assignments are split to avoid new false positives. Local pinned Semgrep reports zero findings (38 existing parser warnings); pinned Gitleaks scans 541 commits with no unignored findings. The seven transfer tests pass. The extended out-of-tree production build also passes required release journeys, native Windows/macOS Files and Settings interactions, and Preview byte-loading/sandbox checks for image, PDF, audio/video, Markdown, CSV, HTML and text; no unexpected HTTP or browser-console errors remain.

### Gateway startup identity race

GitHub's concurrent-start test exposed a real startup race: a script launcher can exec an interpreter with the same PID/start ticks after MSO captures its initial executable/argv identity. Persisting that stale identity let a second start treat the live tunnel as absent and create another. After public readiness, MSO now revalidates the exact command and owned process lifetime, captures the final identity, and only then commits state. A delayed interpreter-exec fixture reproduced two live tunnels before the fix; both direct and interpreter starts now require exactly one spawned/live tunnel whose full identity matches the saved state. Login return paths additionally reject paths that normalize into scheme-relative external destinations, including encoded dot segments.

## 2026-09-09 — Instance portability and MCP privacy boundary

MSO source no longer owns an operator hostname, home path, project root, MCP widget origin, or external Page app. Browser origin comes from `OS_PUBLIC_ORIGIN`; the MCP Apps sibling origin is derived per installation or overridden with `OS_MCP_UI_ORIGIN`; reviewed external Page apps are an empty-by-default, bounded `MSO_SURFACE_APPS_JSON` catalog. Current repository/worktree instructions resolve the active checkout rather than assuming a maintainer home. A repository-wide instance-literal gate now rejects known operator-instance domains/paths across runtime code, tests, docs, workflows, and official skills. Maintainer deployments keep their own public values only in ignored/local runtime configuration.

## 2026-09-08 — Native desktop Files and Settings

Files now presents Finder and File Explorer names and distinct toolbars, navigation, search, folder colors, selection and status treatments while retaining the shared host API. Windows Settings replaces horizontal tabs with a searchable Fluent sidebar; macOS System Settings gains grouped search and colored categories. App names follow the current shell without changing routes or custom file titles. Running Windows taskbar pins now minimize/restore the front existing window instead of duplicating a multi-window app; New window remains explicit.

Browser checks exposed two underlying issues: home-relative breadcrumbs invented /~ paths, and app sidebars only adapted to the viewport. Home paths now remain home-relative, and the shared sidebar switches to a sheet below 600px of actual window width (including Code's existing sidebar). Windows Enter opens, F2 renames and Backspace navigates back; Finder Enter renames. Settings wallpaper previews now have a real width and resolve Auto to the shell backdrop. Tests cover breadcrumb paths, platform profiles and authenticated/preview browser search, rename, taskbar reuse and narrow-window navigation in scripts/e2e/desktop-native.mjs.

## 2026-09-07 — Private named connections for modular project MCPs

A project MCP declaration previously supplied only a URL or raw header values, so Baton discovery through MSO returned 401 despite the generic dispatcher existing. The Project MCP integration now keeps an endpoint-bound downstream bearer in the owner-private connection store; repo manifests carry only a named user/connection reference. Exact optional tool grants apply at discovery and call time, connection changes are rechecked, errors/results redact the selected token, HTTP requests time out, and paginated catalogs fail explicitly when incomplete. Token identity and live downstream RBAC remain authoritative; no impersonation or incoming-token passthrough is added. Shared project metadata types replace a copied auth union. See PROJECT-MCP-CONNECTIONS.md for setup and acceptance. The global tool names remain unchanged; compact discovery copy is shorter to retain scanner headroom after the provider enum grows.

## 2026-09-06 — Dokploy public build env rollout is a bounded native integration operation

The verified native `rahmanfakhr / dokploy / default` connection can now resolve applications within one exact Dokploy project and update one browser-public build environment variable before queueing a redeploy. The mutation accepts only `NEXT_PUBLIC_`, `VITE_`, `PUBLIC_`, `REACT_APP_`, or `EXPO_PUBLIC_` keys, rejects multiline/oversized values and duplicate target keys, preserves existing server env/build args/build secrets privately, re-reads the application to verify the exact change, and redeploys only when the value changed. It does not expose environment contents or add a generic Dokploy request escape hatch. The mutation stays out of the compact ChatGPT global profile and is reached through the existing native `integration_execute` surface.
...
MSO blue, status colors, section/path/filter chrome, a populated Inspector/Result area and a compact contextual shortcut footer make the screen part of the MSO TUI system rather than a generic monochrome picker. `/` filters only the active column; selection/query state is remembered per layer; `V/R/S/D/N`, section keys, transfer shortcuts and help are context-aware. Transfer and Hostinger Mail remain part of the native tree. Presentation state contains metadata only; secrets still enter through the existing private browser/TTY boundary and all mutations use the same Integrations API. PTY acceptance covers 60×20 through 200×50 and treats border continuity/right-edge wrap as release blockers.
...
- **Default-branch security evidence (2026-09-05):** a screenshot still showed 21 open code-scanning alerts despite green analysis jobs. Add a read-only repository-native inventory with exact default-branch SHA, all paginated alert instances, recent analysis metadata and a nonzero exit while any finding remains. External owner API credentials currently fail authentication; the native Actions token has security-events read permission and is never exported. This captures the actual remediation backlog without disabling scanners or dismissing alerts.
- **Native integration preservation (2026-09-05):** merged published c164dbe without dropping the concurrent native credential setup. The short README retains its documentation link (99 lines). MCP 1.12.0 / toolset 2026.09.05.4 labels the combined maintenance, service-token, Page-readiness and native-integration contracts. This merge does not execute maintenance or reset real credentials.
- **Concurrent main reconciliation (2026-09-05):** preserved published MCP Page initialization/readiness changes e46c417 + changelog c4856d2 in the maintenance/PR integration branch without modifying the other session's unpublished work. MCP 1.11.1 / toolset 2026.09.05.2 records the combined compatible contract. The Settings maintenance files are unchanged from the isolated desktop/mobile acceptance at 2164d29; full repository and GitHub gates are rerun for the combined source.
- **PR integration validation (2026-09-05):** preserve the reviewed PR36/PR37 ancestry while repairing PR38 glob escaping/native Bun audit checks and PR37 code-generating visual-test fixtures. Direct-image boundary tests now cover malformed content and size/type limits. MCP server 1.11.0 / toolset 2026.09.05.1 records the compatible restricted-service-token/direct-image contract; widget resources and ordinary OAuth token behavior are unchanged. Publication still requires complete current-head local and remote gates, including late CodeQL findings—not only the workflow completion status.
- **Reviewed 9Router network boundary (PR36, 2026-09-05):** reuse the configured internal proxy network only when it already exists; Docker host publishing remains loopback-only unless explicitly enabled. This supports a containerized reverse proxy without opening the application port to the public interface. The update adds no automatic network creation or DNS mutation. The old PR lifecycle-test failure must be rechecked against the combined current baseline before publication.
...
- **MCP Apps presentation unification / Surface v4 (2026-09-04):** MCP server `1.9.3` / toolset `2026.09.04.4` retires the specialized `project-status-v2`, `project-diff-v2`, and `vps-status-v2` resources. `project_get`, `project_diff`, and `vps_status` are pure data tools again; `ui://mso/surface-v4.html` is the only general-purpose UI and uses the portable MCP Apps `tools/call` bridge for native monitor/project/diff views. `workflow-progress-v2` remains intentionally separate for live workflow polling. Play Together keeps the exact `game.example.com/embed` frame boundary, so the external-iframe review advisory remains expected only for the universal Surface.
...
- **Play Together secure iframe / Surface v3 (2026-09-04):** MCP server `1.9.2` / toolset `2026.09.04.3` upgrades `ui://mso/surface-v3.html` so Play Together can render live inside ChatGPT without relaxing the whole app. The Surface allowlists only `https://game.example.com`, starts it at `/embed`, revalidates the exact origin plus `/embed` path prefix in-browser, and retains a sandbox without popups/top-navigation. The Play Together deployment reciprocally keeps normal routes anti-frame and gives only `/embed` + `/embed/*` the exact `mso-ui.example.com` ancestor permission. Auth remains normal; no demo bypass or raw HTML/arbitrary URL input is added.
- **MSO Surface v1 scope correction / v2 resource (2026-09-04):** MCP server `1.9.1` / toolset `2026.09.04.2` removes accidental cross-project catalog entries from Fresh 3. `ui://mso/surface-v2.html` keeps the generic secure Surface architecture but its current VPSKU app catalog contains only **Play Together** (`https://game.example.com`) as a `remote` target because that app currently sends `frame-ancestors 'self'`. The Surface CSP therefore has **no third-party `frameDomains` at all**. Raw HTML/arbitrary URLs remain rejected, project anti-framing headers are never stripped, and app identities from other MSO server scopes are not copied into this connector.
...
- **Fresh 3 completion / scanner-clean contract (2026-09-03):** server `1.8.1` / toolset `2026.09.03.6` keeps the 62-action Fresh 3 surface unchanged while closing the post-refresh metadata gaps. Every ChatGPT action now advertises `outputSchema`; dynamic actions share one DRY bounded `{ result }` envelope instead of 57 duplicated hand schemas, keeping the profile at 64,180 bytes (~16.0k rough tokens). Four MCP Apps now declare the dedicated sibling widget origin `https://mso-ui.example.com` (Traefik config in `ops/traefik/mso-ui.yml`), and the OpenAI/MCP skills extension publishes five official, digest-verified `claude-skills/` snapshots through `skills/list`, `skills/get`, and `resources/read`. Generic MCP clients retain the prior text-only result behavior unless a tool already had explicit structured output.
...
with `{id}.mso.example.com`, that replaced the real in-shell dashboard with a card that only
...
`9router.mso.example.com` was added to the Traefik managed-apps file and the wildcard DNS
...
- **`CLAUDE.md` documented a demo that does not exist** — no `mso-demo.service`, no
  `/home/operator/projects/mso-demo`. The `NEXT_PUBLIC_OS_DEMO=1` flag is still real; the
...
is a VPS cockpit and `ls /home/operator` must not open a menu.
...
- **Origin split** (`d880f68`) — the iframe needs `allow-same-origin` or the SPAs do not boot, and
  ...
  (`{id}.mso.example.com`, DNS + `/etc/dokploy/traefik/dynamic/mso-managed-apps.yml`), so
...
- **Verified live**, not just unit-tested: `https://hermes.mso.example.com/` and
...
### 2026-09-06 — Convex environment presence checks
