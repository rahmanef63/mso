...
- **Vector and graph complexity remain evidence-blocked:** realistic retrieval calibration is **4/6 lexical**, with objective misses only for `IDE`↔`editor` synonym and Indonesian `kantor`↔English `office`; no embedding layer is added because no candidate has yet proven better recall with acceptable privacy/latency/context cost. Relationship calibration is **3/3 two-hop**, so current keyed retrieval does not justify graph storage.
