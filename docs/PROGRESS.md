# mso — Progress Log
- **MSO Surface v1 / ChatGPT presentation target (2026-09-04):** MCP server `1.9.0` / toolset `2026.09.04.1` adds `mso_surface_apps_list` and `render_mso_surface`, taking the ChatGPT profile to 64 transport tools (63 model actions + app-only `workflow_status`) and the full model catalog to 87. `ui://mso/surface-v1.html` is a universal responsive MCP App with native home/monitor/project/diff/browser routes, fullscreen/PiP controls, and reviewed app-demo routing. Security is default-deny: no raw HTML or arbitrary URL tool inputs, dynamic strings use DOM text nodes, iframe URLs are revalidated against a catalog bundled in the UI resource, nested `frameDomains` contains only exact reviewed origins, sandbox grants omit popups/top-navigation, authenticated MSO remains non-frameable, and user-installed `runtime:"html"`/HTML widgets cannot promote themselves into the ChatGPT frame allowlist. AntiNRML Builder is the first direct live-demo origin; Baton and AntiNRML Game remain remote because their production XFO/CSP currently deny third-party framing.
