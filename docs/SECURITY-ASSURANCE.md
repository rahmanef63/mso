...
The production baseline keeps five rule-wide passive signals visible as `INFO`: cache policy on the public manifest, intentionally non-storable private/auth-shell responses, broad HTTPS sources in `img-src`/`frame-src`, a Unix timestamp in a static bundle, and the deliberate absence of COEP. A sixth reviewed signal, ZAP rule 10050 (`Retrieved from Cache`), is deliberately **not** present in the rule-wide policy. `scripts/check-zap-report.mjs` accepts that rule only when every instance is a same-origin immutable public asset: query-free `/_next/static/chunks/*.js`, `/_next/static/chunks/*.css`, or `/_next/static/media/*.woff2`, plus `/icon.svg` only with Next's bounded `?icon.<hash>.svg` cache-busting shape. Any other 10050 instance fails the gate. Authenticated and user-specific responses remain `private, no-cache, no-store`.
