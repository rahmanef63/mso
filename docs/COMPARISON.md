...
Last reviewed: **2026-09-17** · freshness gate: **90 days**
...
| **Coolify** | Self-hosted Git/PaaS delivery | Partial | Partial | Strong | Partial | Strong | Partial |
| **Portainer** | Container and stack operations | Partial | Partial | Strong | Partial | Partial | Not offered |
| **Netdata** | High-resolution observability and RCA | Partial | Not offered | Not offered | Strong | Partial | Strong |
| **Tailscale SSH** | Identity-aware private remote access | Not offered | Partial | Not offered | Not offered | Strong | Not offered |
| **File Browser** | Archived focused web file management | Partial | Not offered | Not offered | Not offered | Partial | Not offered |
...
- **Browser workspace (Strong):** Files, real PTY, code/media editors, managed apps, Integrations, Workflows, Organization, responsive shells, PWA, and CLI share one authenticated runtime. Evidence: `app/os-root.tsx`, `frontend/slices/files-manager/app.tsx`, `frontend/slices/os-terminal/components/pty-terminal.tsx`, `frontend/slices/workflows/app.tsx`, `frontend/slices/organization/app.tsx`, `frontend/slices/appshell/responsive/responsive-provider.tsx`.
...
- **Application delivery (Partial):** Pinned managed-app install/update/backup/rollback paths are built in, and named Integrations can orchestrate GitHub/Dokploy delivery plus DNS/provider operations. MSO still delegates the underlying PaaS/container control plane rather than replacing one. Evidence: `lib/managed-apps/install.ts`, `lib/managed-apps/update.ts`, `lib/managed-apps/backups.ts`, `lib/infra/dokploy.ts`, `lib/infra/connection-dispatch.ts`.
...
- **Built-in AI and MCP (Strong):** BYOK plus dynamic provider discovery, structured host/editor/integration/workflow tools, OAuth/PKCE Streamable-HTTP MCP, ChatGPT list/page surfaces, durable agent sessions, and project-aware MCP discovery are native product surfaces. Evidence: `app/api/assistant/route.ts`, `app/mcp/route.ts`, `lib/mcp/tools.ts`, `app/api/v1/agent-sessions/route.ts`, `frontend/slices/workflows/app.tsx`.
...
- **Built-in AI and MCP (Partial):** Coolify now exposes a native team-scoped MCP server with tools, resources, prompts, deployment/lifecycle operations, logs, and Git integration discovery. Its documented MCP flow is client-driven through compatible external AI clients rather than an integrated Coolify assistant surface.

Official sources:
- [Coolify features](https://coolify.io/docs)
- [Coolify CI/CD](https://coolify.io/docs/applications/ci-cd)
- [Coolify MCP](https://coolify.io/docs/mcp/what-is-mcp)
...
- [Portainer stack delivery](https://docs.portainer.io/user/docker/stacks/add)
...
- **Roles and delegation (Partial):** Team management exists, and paid plans add RBAC and SSO; identity and remote host access are not Netdata’s primary product scope.
- **Built-in AI and MCP (Strong):** Netdata AI provides built-in conversations, insights and investigations, while native Cloud and Agent/Parent MCP servers expose metrics, alerts, logs, anomaly/RCA queries, and supported function execution to AI clients.
...
- [Netdata AI](https://learn.netdata.cloud/docs/netdata-ai)
- [Netdata MCP](https://learn.netdata.cloud/docs/netdata-ai/mcp)
- [Netdata access control](https://learn.netdata.cloud/docs/security-and-privacy-design/access-control-and-feature-availability)
...
**Best fit:** Archived focused web file management

- **Browser workspace (Partial):** The final shipped product provides upload/delete/preview/edit file management, but the repository is now archived and no longer maintained.
