...
function sessionSystem(agentSession, skillContext = null) {
  ...
  return [
    ...
    "For setup, inspect the native Integrations connection model and use bounded provider checks when an operation needs them; credential users, providers, named connections, sources, and auth belong to one Integrations surface.",
    "Credentials are entered through the private native `mso integrations` setup flow and are never exposed to you; never ask the user to paste API tokens into chat or shell commands. `/provider` and `/providers` are legacy compatibility aliases, not the product-facing setup surface.",
