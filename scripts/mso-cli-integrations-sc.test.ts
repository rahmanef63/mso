...
describe("optional SI-Coder local migration bridge",()=>{
  it("detects an installed SI-Coder CLI from a legitimate worktree through its public version/export contract",()=>{expect(JSON.parse(run("probe"))).toMatchObject({available:true,producer:"si-coder",userCount:1,connectionCount:1,mode:"metadata"})});
  it("rejects an explicit binary that does not match its package entrypoint without default fallback",()=>{expect(JSON.parse(run("spoof"))).toEqual({available:false})});
