...
try {
  for (const legacy of [false, true]) {
    ...
    let childLoads = 0;
    await page.route("https://game.example.com/embed", (route) => {
      childLoads++;
    ...
    await page.route("https://mso-ui.example.com/qa", (route) => route.fulfill({ contentType: "text/html", body: `${legacy ? `<script>window.openai={toolOutput:${JSON.stringify({ structuredContent: output })}}</script>` : ""}${resource.text}` }));
    await page.route("https://chatgpt.com/qa", (route) => route.fulfill({ contentType: "text/html", body: `<!doctype html><script>
      ...
      window.addEventListener("message",event=>{
        ...
        if(msg.method==="ui/initialize"&&!${legacy})frame.postMessage({jsonrpc:"2.0",id:msg.id,result:{protocolVersion:"2026-01-26",hostCapabilities:{serverTools:{},openLinks:{}},hostContext:{displayMode:"inline",maxHeight:800}}},"https://mso-ui.example.com");
        if(msg.method==="ui/notifications/initialized"){
          frame.postMessage({jsonrpc:"2.0",method:"ui/notifications/tool-input",params:{arguments:{route:result.route}}},"https://mso-ui.example.com");
          frame.postMessage({jsonrpc:"2.0",method:"ui/notifications/tool-result",params:{structuredContent:result}},"https://mso-ui.example.com");
        }
        if(msg.method==="ui/notifications/size-changed")document.querySelector("iframe").style.height=msg.params.height+"px";
        if(msg.method==="ui/open-link")frame.postMessage({jsonrpc:"2.0",id:msg.id,result:{}},"https://mso-ui.example.com");
      });</script><iframe src="https://mso-ui.example.com/qa" style="width:100%;height:740px"></iframe>` }));
    ...
    if (!legacy) {
      ...
      const hostHeight = await page.locator('iframe[src="https://mso-ui.example.com/qa"]').evaluate((node) => node.getBoundingClientRect().height);
    ...
    const widget = page.frames().find((frame) => frame.url() === "https://mso-ui.example.com/qa");
    const game = page.frames().find((frame) => frame.url() === "https://game.example.com/embed");
    assert(widget && game);
    if (!legacy) {
    ...
    await widget.evaluate(() => window.dispatchEvent(new MessageEvent("message", { origin: "https://game.example.com", source: window, data: { type: "play-together:embed-ready", schemaVersion: 1 } })));
    assert.equal(await component.locator(".frame-wrap").getAttribute("data-state"), "loading"); assertions++;
    await game.evaluate(() => parent.postMessage({ type: "play-together:embed-ready", schemaVersion: 99 }, "https://mso-ui.example.com"));
    assert.equal(await component.locator(".frame-wrap").getAttribute("data-state"), "loading"); assertions++;
    ...
    await component.locator('.frame-wrap[data-state="loading"]').waitFor();
    await page.waitForTimeout(50);
    const reloaded = page.frames().find((frame) => frame.url() === "https://game.example.com/embed");
    assert(reloaded); assertions++;
    await reloaded.evaluate(() => parent.postMessage({ type: "play-together:embed-ready", schemaVersion: 1 }, "https://mso-ui.example.com"));
    await component.locator('.frame-wrap[data-state="ready"]').waitFor(); assertions++;
    const previousLoads = childLoads;
    await page.evaluate(() => document.querySelector("iframe").contentWindow.postMessage({ jsonrpc: "2.0", method: "ui/notifications/tool-result", params: { structuredContent: window.result } }, "https://mso-ui.example.com"));
    ...
    if (!legacy) {
      ...
      assert((await page.evaluate(() => window.calls)).some((call) => call.method === "ui/open-link" && call.params.url === "https://game.example.com")); assertions++;
    }
    ...
    await reloaded.evaluate(() => parent.postMessage({ type: "mso:app-auth-request", schemaVersion: 1, provider: "google" }, "https://mso-ui.example.com"));
    await page.waitForTimeout(50);
    ...
    if (!legacy) {
      ...
      assert((await page.evaluate(() => window.calls)).some(call => call.method === "ui/open-link" && call.params.url === "https://game.example.com/?auth=google")); assertions++;
    }
