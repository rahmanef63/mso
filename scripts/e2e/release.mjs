...
execFileSync(process.execPath, ["scripts/e2e/graph-custom.mjs"], { stdio: "inherit" });
...
try {
  ...
  await expect(page.getByText("Release fixture · fixture", { exact: true })).toBeVisible();
  await page.getByLabel("Export scope").selectOption("all");
  await page.getByLabel("Export format").selectOption("env");
  await page.getByLabel("MSO owner password").fill(fixture.password);
  await page.getByLabel("I understand this download contains plaintext credentials").check();
  const rawDownloadPromise = page.waitForEvent("download");
  await page.getByRole("button", { name: "Re-authenticate & download", exact: true }).click();
  const rawDownload = await rawDownloadPromise;
  expect(rawDownload.suggestedFilename()).toMatch(/\.env$/);
  await expect(page.locator('.portable-transfer [role="status"]')).toContainText("one-time export grant is now invalid");
