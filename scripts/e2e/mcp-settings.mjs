...
export async function mcpOwnerJourneys(page, fixture) {
  ...
  for (const viewport of [{ width: 1363, height: 936 }, { width: 390, height: 844 }, { width: 844, height: 390 }]) {
    ...
    await siCoder.getByRole("button", { name: "Install", exact: true }).click();
    const installed = page.getByRole("article").filter({ has: page.getByRole("heading", { name: "si-coder", exact: true }) });
    await expect(installed.getByRole("button", { name: "Uninstall", exact: true })).toBeVisible();
