...
export async function mcpOwnerJourneys(page, fixture) {
  ...
  for (const viewport of [{ width: 1363, height: 936 }, { width: 390, height: 844 }, { width: 844, height: 390 }]) {
    ...
    await nav.getByRole("button", { name: "MCP overview", exact: true }).click();
    await page.getByRole("tab", { name: /MSO Access/ }).click();
    const outbound = page.getByRole("navigation", { name: "MCP navigation" });
    await expect(outbound.getByRole("button", { name: /^Project plugins/ })).toBeVisible();
    await expect(page.getByText("Each project starts with no SI-Coder or Batonly binding.", { exact: false })).toBeVisible();
    await outbound.getByRole("button", { name: /^Project plugins/ }).click();
    await page.getByLabel("Plugin target project").fill(fixture.dir);
    const siCoder = page.getByRole("listitem").filter({ hasText: "SI-Coder" });
    await siCoder.getByRole("button", { name: "Check installation", exact: true }).click();
    await expect(siCoder.getByText("Not installed in this project.", { exact: true })).toBeVisible();
    await siCoder.getByRole("button", { name: "Install to project", exact: true }).click();
    await expect(siCoder.getByText("Installed in this project.", { exact: false })).toBeVisible();
    page.once("dialog", dialog => dialog.accept());
    await siCoder.getByRole("button", { name: "Uninstall", exact: true }).click();
    await expect(siCoder.getByText("Not installed in this project.", { exact: true })).toBeVisible();
