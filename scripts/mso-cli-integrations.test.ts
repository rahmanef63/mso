...
const script=path.join(process.cwd(),"scripts/mso-cli-integrations.sh");
function run(args:string[]){return execFileSync("bash",["-c",`set -e
source "$1"
shift
enc(){ printf '%s' "$1"; }
jget(){ printf '%s' "$1"; }
...
tty_ok(){ return 1; }
run_integrations "$@"
`,"test",script,...args],{encoding:"utf8"});}
