...
function edgeVisual(edge:WorkflowGraph["edges"][number],state:WorkflowGraphRunEdgeState|undefined,nodeStates?:ReadonlyMap<string,WorkflowGraphNodeState>){
  ...
  return {animated:running&&!disabled,style:{stroke,strokeWidth:running?2.5:state==="enabled"?2:1.5,opacity:disabled ? 0.24 : state === "pending" ? 0.55 : 1,strokeDasharray:disabled?"2 6":branch?"7 5":undefined},markerEnd:{type:MarkerType.ArrowClosed,width:18,height:18,color:stroke}};
}
...
export function WorkflowCanvas({graph,selectedId,onSelect,onMove,onConnect,nodeStates,edgeStates,onOpen,onDeleteNodes,onDeleteEdges,onTidy,readOnly=false}:{graph:WorkflowGraph;selectedId:string|null;onSelect:(id:string|null)=>void;onMove:(id:string,position:{x:number;y:number})=>void;onConnect:(source:string,target:string,sourceHandle?:string)=>void;nodeStates?:ReadonlyMap<string,WorkflowGraphNodeState>;edgeStates?:ReadonlyMap<string,WorkflowGraphRunEdgeState>;onOpen?:(id:string)=>void;onDeleteNodes?:(ids:string[])=>void;onDeleteEdges?:(ids:string[])=>void;onTidy?:()=>void;readOnly?:boolean;}){
  ...
  const compactFitNodeIds=useMemo(()=>{if(readOnly)return graph.nodes.slice(0,9).map((node)=>node.id);const triggers=graph.nodes.filter((node)=>["manual","schedule","webhook"].includes(node.type)).map((node)=>node.id);return (triggers.length?triggers:[graph.nodes[0]?.id]).filter((id):id is string=>Boolean(id)).slice(0,2);},[graph.nodes,readOnly]);
  ...
  return <GraphCanvas<FlowNode,FlowEdge> ariaLabel="Workflow canvas" showMinimap={!readOnly} nodes={nodes} edges={edges} nodeTypes={nodeTypes} onNodesChange={onNodesChange} onEdgesChange={onEdgesChange} onConnect={readOnly?undefined:connect} onPaneClick={()=>onSelect(null)} onNodeClick={(_,item)=>onSelect(item.id)} onNodeDoubleClick={(_,item)=>{const node=item.data.node;if((readOnly||node.type==="project"||node.type==="folder")&&onOpen)onOpen(node.id);}} onNodeDragStop={(_,item)=>onMove(item.id,item.position)} onNodesDelete={readOnly?undefined:(items)=>onDeleteNodes?.(items.map((item)=>item.id))} onEdgesDelete={readOnly?undefined:(items)=>onDeleteEdges?.(items.map((item)=>item.id))} isValidConnection={(connection)=>Boolean(connection.source&&connection.target&&connection.source!==connection.target&&!graph.edges.some((edge)=>edge.source===connection.source&&edge.target===connection.target&&(edge.sourceHandle??undefined)===(connection.sourceHandle==="output"?undefined:connection.sourceHandle??undefined)))} nodesDraggable={!readOnly} nodesConnectable={!readOnly} edgesReconnectable={false} deleteKeyCode={readOnly?null:["Backspace","Delete"]} onTidy={onTidy} snapToGrid snapGrid={[12,12]} defaultEdgeOptions={{type:"smoothstep",markerEnd:{type:MarkerType.ArrowClosed}}} miniMapNodeColor={(node)=>minimapColor(node as FlowNode)} miniMapNodeStrokeColor={(node)=>node.selected?"var(--accent)":"var(--border)"} compactFitNodeIds={compactFitNodeIds} compactFitMaxZoom={readOnly?0.72:0.9}/>
}
