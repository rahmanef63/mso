    ...
    <div role="status" aria-live="polite" className={cn(
      ...
      <div className="flex items-start gap-2">
        ...
        <div className="min-w-0">
          <p className="text-sm font-medium">{ready ? "MCP + OAuth discovery ready" : state === "error" ? "One or more connection checks failed" : "Checking MCP + OAuth discovery…"}</p>
          <p className="mt-0.5 break-all font-mono text-xs text-muted-foreground">{endpoint}</p>
        </div>
      </div>
      {result && (
        <div className="flex flex-wrap gap-1.5 pl-6">
          {result.checks.map((check) => (
            <span key={check.id} className={cn("rounded-md px-2 py-1 text-xs font-medium", check.ok ? "bg-success/10 text-success" : "bg-destructive/10 text-destructive")}>{check.ok ? "Ready: " : "Failed: "}{MCP_PROBE_LABELS[check.id]}</span>
          ))}
        </div>
      )}
    </div>
