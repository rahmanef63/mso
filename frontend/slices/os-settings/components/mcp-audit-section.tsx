    ...
    >
      <SettingsBlock className="p-0">
        <div className="flex min-h-[46px] items-center justify-between gap-3 border-b border-border/60 px-4 py-2.5">
          <div>
            <p className="text-sm font-medium">Privileged MCP log</p>
            <p className="text-xs text-muted-foreground">Newest events first</p>
          </div>
          <span className="shrink-0 rounded-md bg-secondary px-2 py-1 text-xs text-muted-foreground">Latest {trail.length}</span>
        </div>
        {trail.length === 0 ? (
                ...
                return (
                  <li key={`${entry.ts ?? "event"}-${index}`} className="grid grid-cols-[auto_minmax(0,1fr)] gap-x-2.5 gap-y-1 px-4 py-3 sm:grid-cols-[auto_minmax(0,1fr)_auto]">
                    ...
                    <div className="min-w-0">
                      <code className={failed ? "font-mono text-sm font-medium text-destructive" : "font-mono text-sm font-medium"}>{entry.action}</code>
                      {(entry.target || entry.detail) && (
                        <p className="mt-0.5 line-clamp-2 break-all font-mono text-xs leading-relaxed text-muted-foreground">{entry.target ?? entry.detail}</p>
                      )}
                      {entry.actor && <p className="mt-0.5 truncate text-xs text-muted-foreground">{entry.actor}</p>}
                    </div>
                    <time className="col-start-2 shrink-0 text-xs text-muted-foreground sm:col-start-3 sm:row-start-1">
