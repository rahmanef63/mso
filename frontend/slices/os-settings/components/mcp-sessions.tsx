...
export function McpSessions() {
  ...
  function openSession(id: string) { openWindow("workflows", "Workflows", undefined, { view: "sessions", sessionId: id }); }
  return <div ref={top} tabIndex={-1} className="min-w-0 space-y-4 outline-none">
    {guide ? <McpSessionGuide onBack={() => { setGuide(false); focusTop(); }} /> : <>
        ...
        <p className="text-sm text-muted-foreground">Saved MCP, CLI, and Alfa sessions. Selecting one opens its read-only execution graph in Workflow; internal session IDs stay hidden from the interface.</p>
        <form className="flex gap-2" onSubmit={event => { event.preventDefault(); setQuery(search.trim()); setPage(1); }}><Input type="search" aria-label="Search saved sessions" placeholder="Search agent-context or project" maxLength={200} value={search} onChange={event => { setSearch(event.target.value); if (!event.target.value) { setQuery(""); setPage(1); } }} /><Button type="submit" variant="outline">Search</Button></form>
        ...
        {!data ? !error && <p role="status">Loading sessions…</p> : <>
            ...
            {data.sessions.length ? <div aria-label="Session cards" className="flex min-w-0 flex-col divide-y divide-border">
              {data.sessions.map(session => <Button key={session.id} variant="ghost"
                aria-label={`Open session ${session.label}`} onClick={() => openSession(session.id)}
                ...
                <span className="w-full truncate text-sm font-semibold">{session.label}</span>
                <span className="line-clamp-2 w-full break-words text-xs font-normal text-muted-foreground [overflow-wrap:anywhere]">{session.title}</span>
                <span className="text-xs font-normal text-muted-foreground">{session.eventCount} events · {session.receiverConnected ? "Receiver connected" : "No receiver connected"}</span>
                <span className="text-xs font-normal text-muted-foreground">Last seen {sessionTime(session.lastSeenAt)}</span>
              </Button>)}
            </div> : <SettingsBlock><p className="text-sm">{query ? "No sessions match this search. Try the agent name, context, or project." : includeOffline ? "No stored sessions yet. Connect an MCP client or start an MSO CLI agent." : "No active sessions right now. Open All stored to inspect earlier sessions."}</p></SettingsBlock>}
            <SessionPaging {...data} label="Sessions" onPage={next => { setPage(next); focusTop(); }} />
            {includeOffline && <p className="text-xs text-muted-foreground">Shows readable saved sessions; archived transcripts remain separate. Plain terminal shells without an MSO agent session are not listed.</p>}
          </>}
      </>}
  </div>;
}
