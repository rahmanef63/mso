...
export function useSessionMonitor<T>(query: string) {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      try {
        ...
        if (!response.ok) throw new Error(response.status === 403 ? "Owner access is required. Sign in again." : response.status === 404 ? "This session is no longer available." : "Sessions could not be loaded. Try again.");
        const data = await response.json() as T;
        if (live && current === controller) setResult({ key, data });
      } catch (error) {
        if (live && current === controller) setResult({ key, error: error instanceof Error && error.name !== "AbortError" ? error.message : "Session request timed out. Try again." });
  ...
  return { ...(result?.key === query + ":" + attempt ? result : {}), reload: () => retry(value => value + 1) };
}
