...
const req=(body:unknown)=>new NextRequest("https://mso.example.com/api/v1/integrations",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(body)});
it("enforces Owner on profile metadata and mutations regardless of selected credential user",async()=>{
  const {GET,POST}=await import("./route");for(const role of [null,"viewer","operator"]){auth.role=role;expect((await GET(new NextRequest("https://mso.example.com/api/v1/integrations"))).status).toBe(403);expect((await POST(req({mode:"manage",action:"user.create",confirm:true,user:"owner"}))).status).toBe(403);}
});
it("shares confirmed actions with the CLI/MCP core and rejects secret-shaped metadata",async()=>{
  ...
  const response=await GET(new NextRequest("https://mso.example.com/api/v1/integrations?view=users"));expect(await response.json()).toMatchObject({users:[{id:"alice"}],user:"alice"});
});
