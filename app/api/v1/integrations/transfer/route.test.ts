...
let root:string;
beforeEach(async()=>{root=await fs.mkdtemp(path.join(os.tmpdir(),'mso-transfer-route-'));process.env.OS_INFRA_STORE=path.join(root,'infra.json');process.env.OS_PUBLIC_ORIGIN='https://mso.example.test';auth.role='owner';vi.resetModules()});
afterEach(async()=>{delete process.env.OS_INFRA_STORE;delete process.env.OS_PUBLIC_ORIGIN;await fs.rm(root,{recursive:true,force:true});vi.resetModules()});
