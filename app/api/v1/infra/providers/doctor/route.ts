...
export async function POST(req: NextRequest) {
  ...
  let body: { id?: string; user?: string; connection?:string; cwd?:string } = {};
  ...
  const results = await withIntegrationSelection(selectionFrom(body),()=>Promise.all(ids.map((id) => doctorInfraProvider(id as (typeof INFRA_PROVIDER_IDS)[number]))));
