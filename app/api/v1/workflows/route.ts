import { learningPage } from "@/lib/workflow/learning-page";
...
export async function GET(req:NextRequest){const q=req.nextUrl.searchParams,minimum=(q.has("variables")||q.has("data_tables")||q.has("data_table"))?"operator":"viewer",session=await auth(minimum);if(!session)return fail("unauthorized",401);try{
 ...
 if(q.get("learning")==="1"){if(!roleAtLeast(session.context.role,"owner"))return fail("owner_required",403);return NextResponse.json(await learningPage({offset:Number(q.get("offset"))||0,limit:Number(q.get("limit"))||40,includeArchived:q.get("include_archived")==="1",sessionLabel:q.get("session_label")?.slice(0,200)||undefined}),{headers});}
...
export async function POST(req:NextRequest){const session=await auth("operator");if(!session)return fail("operator_required",403);try{const body=await readSetupJson(req),action=typeof body.action==="string"?body.action:"";if(rateLimited(`workflow:${action}:${session.context.session.device_id}`,action==="run"?60:30,60_000))return fail("rate_limited",429);
 if(action==="owner_clone"){if(session.context.role!=="owner")return fail("owner_required",403);if(body.confirm!==true)return fail("explicit_confirmation_required",400);return NextResponse.json({graph:await cloneOwnerGraph(session.context.role,session.principal,String(body.origin_owner??""),String(body.graph_id??""),String(body.expected_revision??""))},{headers});}
