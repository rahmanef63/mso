...
import { matchesAgentApproval } from "@/lib/agent/approval.js";
...
export async function POST(req: NextRequest) {
  const context = await ownerContext();
  if (!context) return NextResponse.json({ error: "owner_role_required" }, { status: 403 });
  let body: { name?: string; input?: Record<string, unknown>; approved?: boolean; approvalDigest?: string };
  ...
  const rpc = await dispatch({ id: "cli-call", method: "tools/call", params: { name, arguments: body.input ?? {} } }, scope, `cli:${context.session.device_id}`);
